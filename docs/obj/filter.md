---
id: search-criteria
title: Searching for learning objects
---

To retrieve learning objects from the EduTech Repository, a ``GET`` HTTP request towards ``/learning-objects/search/`` with parameters in the query string is necessary.
The parameters in the query string can include a number based on the values of one or more of the following search criteria: learning object licensing with ``license__value``, level of education with ``education_levels__id`` , learning and general knowledge area with ``knowledge_area__id``. The id values are mapped to the catalogue of search criteria in the show search criteria section.
...
Failed Response: A non-existing search criterion is entered in the Query String.
